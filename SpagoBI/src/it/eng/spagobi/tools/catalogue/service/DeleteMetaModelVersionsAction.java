 ...
 */
public class DeleteMetaModelVersionsAction extends AbstractSpagoBIAction {
	
	public static final String VERSIONS_ID = "id";
	
	// logger component
	private static Logger logger = Logger.getLogger(DeleteMetaModelVersionsAction.class);
	...
	public void doService() {
		...
		try {
			...
			for (int i = 0; i < idArray.length; i++) {
				...
				logger.debug("Deleting Meta model content with id = " + id);
				dao.eraseMetaModelContent(id);
				logger.debug("Meta model content with id = " + id + " successfully deleted");
			}
			try {
